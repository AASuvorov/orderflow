...
def funding_report(*, refresh: bool = True) -> tuple[str, Path]:
    ...
    best_sym = max(data, key=lambda s: annualized(data[s], 30)).replace("USDT", "")
    text = (
        "<b>Фандинг перпетуалов: сколько платит нейтральная позиция</b>\n\n"
        f"<b>Лучший инструмент — {html.escape(best_sym)}, {best:+.1f}% годовых</b> за "
        f"последние 30 дней. Долларовая безрисковая ставка при этом около "
        f"{RISK_FREE_USD_PCT:.0f}%.\n\n"
...
def costs_report() -> tuple[str, Path]:
    ...
    text = (
        ...
        f"<b>Разброс {dear['издержки_круг_бп'] / cheap['издержки_круг_бп']:.1f}× "
        f"между {html.escape(cheap['secid'])} и {html.escape(dear['secid'])}</b>: "
        f"от {cheap['издержки_круг_бп']:.2f} до {dear['издержки_круг_бп']:.2f} "
        f"базисного пункта за вход и выход.\n\n"
...
def spread_report(snapshots_n: int = 10) -> tuple[str, Path]:
    ...
    text = (
        ...
        f"<b>Условие проходят {viable.height} инструментов из {df.height}</b> — "
        f"остальным половины спреда не хватает даже на комиссию.\n\n"
        ...
        "движения цены. Проверил все перпетуалы с оборотом выше 20 млн $ в сутки. "
...
def session_report() -> tuple[str, Path]:
    ...
    text = (
        ...
        # Главное число — сразу после заголовка. В превью и при репосте видно
        # первые строки, и если там описание метода, пост выглядит пустым.
        # Знак определяет слово: «−51.7% перевеса покупок» читается как ошибка.
        f"<b>{html.escape(top['контракт'])}: перевес агрессивных "
        f"{'покупок' if top['дельта_доля_%'] > 0 else 'продаж'} на "
        f"{abs(top['дельта_доля_%']):.1f}%</b> от объёма за сессию"
        + (f", пик в {top['час']}:00 МСК — {abs(top['час_доля']):.1f}%"
           if top["час"] is not None else "")
        + ".\n\n"
        f"Считаю по своим тикам со стороной инициатора сделки — той, кто ударил по "
        f"цене, а не просто стоял в стакане. За сессию {_num(total_trades)} сделок "
        f"по {len(rows)} контрактам.\n\n"
        + "Весь список, % от объёма:\n\n"
        + listing
...
def morning_report() -> tuple[str, Path]:
    ...
    text = (
        ...
        # Расхождение цены и открытого интереса — самый содержательный факт дня и
        # то, чего нет в новостных сводках. Поэтому он идёт первым.
        f"<b>{html.escape(inflow['ASSETCODE'])}: интерес {inflow['интерес_%']:+.1f}% "
        f"при цене {move:+.2f}%.</b> {inflow_note}.\n\n"
        ...
        + movers_txt
        + f"\n\n<b>Ушли из {html.escape(outflow['ASSETCODE'])}</b>: интерес "
...
def week_report() -> tuple[str, Path]:
    ...
    text = (
        ...
        f"<b>{html.escape(inflow['ASSETCODE'])}: открытый интерес "
        f"{inflow['интерес_%']:+.1f}%</b> — больше всех за неделю среди ликвидных "
        f"активов МОЕХ. Обороты резче всех выросли в "
        f"{html.escape(busiest['ASSETCODE'])}: {busiest['оборот_%']:+.1f}%.\n\n"
        "Не пересказ событий, а замеры: что изменилось за неделю там, где я меряю сам.\n\n"
        "<b>Фандинг перпетуалов, неделя к предыдущей:</b>\n"
        + "\n".join(lines)
        + f"\n\n<b>Деньги на МОЕХ.</b> Сильнее всего открытый интерес сократился у "
        f"{html.escape(outflow['ASSETCODE'])}: {outflow['интерес_%']:+.1f}%. "
        f"Полная картина по десяти самым оборотистым активам — на графике.\n\n"
